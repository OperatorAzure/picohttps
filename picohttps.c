...
void main(void){
    ...
    // //adjust your post request here
    // const char PICOHTTPS_BODY =
    // "{\n"
    // "  \"sender\": \"Emily Wong\",\n"
    // "  \"message\": \"Successful HTTPS Post Request to the server.\"\n"
    // "}";

    if(!send_request(pcb, PICOHTTPS_BODY)){
...
// Send HTTP POST request with a JSON body
bool send_request(struct altcp_pcb* pcb, const char *body) {
    ...
    // Write to send buffer
    cyw43_arch_lwip_begin();
    lwip_err_t lwip_err = altcp_write(pcb, request, req_len, 0);
    cyw43_arch_lwip_end();
